...
- **Content Service** — отвечает за потребление контента: история просмотров, прогресс воспроизведения (где пользователь остановился). Агрегирует видеопоток из внешних онлайн-кинотеатров.
- **Movies Service** — каталог: метаданные фильмов, жанры, рейтинги и пользовательские оценки. Взаимодействует с внешней рекомендательной системой. Хранит статику (обложки, постеры) в S3.
